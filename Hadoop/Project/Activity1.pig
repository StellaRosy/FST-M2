-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/stella/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/stella/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/stella/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/stella/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/stella/outputs' USING PigStorage('\t');